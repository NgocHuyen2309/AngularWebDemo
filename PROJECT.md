...
| 10 | **v3.4 Theme & UI Polish** | Per-theme search icon contrast (`#ECB44D` gold in dark mode vs `#3A4A6B` dark slate in light mode), unified dashboard card buttons (`.btn-card-primary`), custom flexbox-centered profile edit overlay (`.profile-modal-overlay` / `.profile-modal-box` stripping conflicting Bootstrap modal classes), high-contrast PrimeNG toast notification solid color schemes (`p-toast`), stripped emoji prefixes from action buttons (`Lock`/`Unlock`/`Promote`/`Demote`), and dynamic hero welcome text shimmer keyframe animations (`#FFFFFF -> #FFF3B0 -> #FFD700 -> #FFFFFF` in Light mode vs `#FFFFFF -> #FEF3C7 -> #BAE6FD -> #FFFFFF` in Dark mode). | M9 | DONE |
