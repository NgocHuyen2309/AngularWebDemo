...
6. **Tính Năng Cập Nhật & Chỉnh Sửa Tài Khoản Trực Tiếp (`v3.2 Inline Edit Feature`)**:
   - **Giao Diện Chỉnh Sửa Mượt Mà (Inline Mode)**: Bổ sung nút cây bút (`.user-list__edit-btn`) ngay kế bên nút thùng rác xóa trong mỗi thẻ tài khoản. Khi nhấn vào cây bút, thẻ tài khoản đó chuyển đổi trực tiếp sang chế độ chỉnh sửa inline (`[class.user-list__item--editing]="editingId === user.id"`).
   - **Đồng Bộ Dữ Liệu Hai Chiều (`FormsModule`)**: Các ô nhập liệu Email và Date of Birth tự động hiển thị dữ liệu hiện tại (`ngModel`), cho phép người dùng tùy chỉnh thông tin nhanh chóng mà không cần popup cồng kềnh.
   - **Kết Nối Backend RESTful API**: Nhấn `Save` sẽ gửi yêu cầu `PUT /api/users/:id` thông qua phương thức `userService.updateUser(id, { email, date_of_birth })`. Khi cập nhật thành công, danh sách lập tức đồng bộ thông báo qua stream `userAdded$` và quay trở về chế độ hiển thị bình thường. Trạng thái lỗi (trùng email, sai định dạng) được hiển thị cảnh báo ngay bên trong thẻ đang chỉnh sửa.
